    ...
    <div class="skrining-wrapper">
        ...
        <header class="page-header">
            <div class="header-left">
                ...
                <p class="page-subtitle" style="margin-top: 4px; color: var(--gray-500); font-size: 14px;">
                    Pemantauan kesehatan & inventaris posyandu lansia
                </p>
            </div>
        </header>
        ...
        <form action="{{ route('skrining.store') }}" method="POST" id="formSkrining"
            ...
            {{-- ══════════════════════════════════════
            STEP 1 — DATA LANSIA + KUNJUNGAN RUTIN
            ══════════════════════════════════════ --}}
                ...
                <div class="form-section">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="preview-imt" class="form-control" readonly
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" name="gula_darah" class="form-control"
                                    placeholder="100" value="{{ old('gula_darah') }}" required>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="kolesterol" class="form-control"
                                    placeholder="180" value="{{ old('kolesterol') }}" required>
                                <small class="form-hint">Baik: &lt;150 · Sedang: 150–189 · Tidak Baik: ≥190</small>
                            </div>
                        </div>

                        <div class="form-group" id="iva-sadanis-group">
                            <label class="form-label">IVA / Sadanis
                                <span class="subsection-hint">(opsional, perempuan)</span>
                            </label>
                        ...
                        <div class="gaya-hidup-item" style="margin-bottom:14px;">
                            ...
                            <div class="radio-group">
                                <label><input type="radio" name="merokok" value="1" required
                                        {{ old('merokok') == '1' ? 'checked' : '' }}> Ya</label>
                                <label><input type="radio" name="merokok" value="0"
                                        {{ old('merokok') == '0' ? 'checked' : '' }}> Tidak</label>
                            </div>
                        </div>
                        ...
                        <div class="form-group" id="paparan-frekuensi-group"
                            ...
                            <div class="radio-group">
                                <label><input type="radio" name="paparan_asap_rokok_frekuensi" value="1"
                                        {{ old('paparan_asap_rokok_frekuensi') == '1' ? 'checked' : '' }}> Setiap Hari</label>
                                <label><input type="radio" name="paparan_asap_rokok_frekuensi" value="2"
                                        {{ old('paparan_asap_rokok_frekuensi') == '2' ? 'checked' : '' }}> Tidak Setiap Hari</label>
                            </div>
                        </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="checkbox-grid">
                            @foreach ($penyakitPpok as $val => $label)
                                <label class="checkbox-item">
                                    <input type="checkbox" name="riwayat_penyakit_keluarga_ppok[]" value="{{ $val }}"
                        ...
                        <div class="checkbox-grid">
                            @foreach ($penyakitSendiriPpok as $val => $label)
                                <label class="checkbox-item">
                                    <input type="checkbox" name="riwayat_penyakit_sendiri_ppok[]" value="{{ $val }}"
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" name="puma_rokok_per_hari" id="puma-rokok-per-hari"
                                    class="form-control" placeholder="0" min="0"
                                    value="{{ old('puma_rokok_per_hari', 0) }}" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Lama merokok (tahun)</label>
                                <input type="number" name="puma_lama_merokok_tahun" id="puma-lama-merokok"
                                    class="form-control" placeholder="0" min="0"
                                    value="{{ old('puma_lama_merokok_tahun', 0) }}" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Pack Years (otomatis)</label>
                            <input type="text" id="preview-pack-years" class="form-control" readonly
                                placeholder="—" tabindex="-1">
                        </div>

                        <div class="subsection-label">
                            Hasil Spirometri Pre-Bronkodilator
                            <span class="subsection-hint">(jika dilakukan)</span>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label class="form-label">VEP1 Pre (liter)</label>
                                <input type="number" name="vep1_pre" id="vep1-pre" step="0.01"
                                    class="form-control" placeholder="2.50" value="{{ old('vep1_pre') }}">
                            </div>
                            <div class="form-group">
                                <label class="form-label">KVP Pre (liter)</label>
                                <input type="number" name="kvp_pre" id="kvp-pre" step="0.01"
                                    class="form-control" placeholder="3.00" value="{{ old('kvp_pre') }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Rasio VEP1/KVP Pre (%) (otomatis)</label>
                            <input type="text" id="preview-rasio-pre" class="form-control" readonly
                                placeholder="—" tabindex="-1">
                        </div>

                        <div class="subsection-label">
                            Hasil Spirometri Post-Bronkodilator
                            <span class="subsection-hint">(jika diberikan bronkodilator)</span>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" name="vep1_post" id="vep1-post" step="0.01"
                                    class="form-control" placeholder="2.50" value="{{ old('vep1_post') }}">
                            </div>
                            <div class="form-group">
                                <label class="form-label">KVP Post (liter)</label>
                                <input type="number" name="kvp_post" id="kvp-post" step="0.01"
                                    class="form-control" placeholder="3.00" value="{{ old('kvp_post') }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Rasio VEP1/KVP Post (%) (otomatis)</label>
                            <input type="text" id="preview-rasio-post" class="form-control" readonly
                                placeholder="—" tabindex="-1">
                        </div>
            ...
            <div class="wizard-panel" id="step-review">
                ...
                <div class="wizard-nav">
                    ...
                    <button type="submit" class="btn-submit-final" id="btn-submit-skrining">
                        <i class="fa-solid fa-floppy-disk"></i> Simpan Semua Skrining
                    </button>
                </div>
            </div>
            ...
            <div style="background:white; border-radius:8px; box-shadow:0 4px 6px rgba(0,0,0,0.1);
                ...
                <div style="padding:16px 20px; border-bottom:1px solid #E5E7EB;
                    ...
                    <h2 style="margin:0; font-size:16px; font-weight:600;">
                        Lansia yang Sudah Skrining Hari Ini
                    </h2>
                ...
                <div style="padding:0; flex:1; overflow-y:auto;">
                    <table style="width:100%; border-collapse:collapse; font-size:13px;">
                        <thead style="background:#F9FAFB; position:sticky; top:0;">
                            <tr>
                                <th style="padding:12px 20px; text-align:left; border-bottom:1px solid #E5E7EB;">
                                    NAMA / NIK
                                </th>
                                <th style="padding:12px 20px; text-align:left; border-bottom:1px solid #E5E7EB; width:150px;">
                                    JENIS KELAMIN
                                </th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    ...
                                    <td style="padding:12px 20px; border-bottom:1px solid #E5E7EB;">
                                        @if ($l->jenis_kelamin == 'L')
                                            <span style="color:#2563EB; background:#DBEAFE; padding:2px 8px;
                                                         border-radius:4px; font-size:11px; font-weight:500;">
                                                Laki-laki
                                            </span>
                                        @else
                                            <span style="color:#DB2777; background:#FCE7F3; padding:2px 8px;
                                                         border-radius:4px; font-size:11px; font-weight:500;">
                                                Perempuan
                                            </span>
                                        @endif
...
@push('scripts')
    <script>
        ...
        document.addEventListener('click', function(e) {
            ...
            if (e.target === modal) closeModalSudahSkrining();
        });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function goToStep(targetId) {
                ...
                setTimeout(() => toPanel.classList.remove('enter-right', 'enter-left'), 20);

                wizSteps.forEach((ws, i) => {
                    ws.classList.toggle('active', i === toIdx);
                    ws.classList.toggle('completed', i < toIdx);
                });
                connectors.forEach((c, i) => c.classList.toggle('filled', i < toIdx));
            ...
            function lockLansia() {
                ...
                if (!sel) return;
                sel.disabled = true;
                if (!document.getElementById('hidden-id-lansia')) {
            ...
            document.querySelectorAll('.btn-next').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    if (target) { goToStep(target); lockLansia(); }
                });
            });
            ...
            function validatePanel(panel, includeHidden = false) {
                ...
                requiredFields.forEach(el => {
                    ...
                    if (el.type === 'radio' || el.type === 'checkbox') {
                        if (!panel.querySelector(`input[name="${el.name}"]:checked`)) {
                            isEmpty = true;
                    ...
                    if (isEmpty && el.type !== 'radio' && el.type !== 'checkbox') {
                ...
                if (!ok) {
                    const first = panel.querySelector(
                        '.field-error, .radio-group.field-error, .checkbox-grid.field-error'
                    );
           ...
           function enableSyncedFieldsBeforeSubmit() {
            ...
            function buildReview() {
                ...
                const lansiaName    = lansiaEl ? lansiaEl.options[lansiaEl.selectedIndex]?.text : '—';
                const pekerjaanDisp = document.getElementById('ppok-pekerjaan-display')?.value || '—';
                ...
                const jmlResep  = adaResep ? document.querySelectorAll('#resep-list .resep-row').length : 0;
                ...
                html += reviewCard('fa-stethoscope', 'Kunjungan Rutin', [
                    ...
                    ['Resep Obat',    adaResep ? jmlResep + ' item' : 'Tidak ada'],
                ]);
                ...
                if (packYears && packYears !== '—' && packYears !== '0 pack-years') {
            ...
            function decodePekerjaan(value) {
                const map = {
                    '1': 'TNI/POLRI', '2': 'PNS', '3': 'Karyawan Swasta',
                    '4': 'Buruh', '5': 'Petani/Nelayan', '6': 'Tidak Bekerja / IRT', '7': 'Lainnya',
                };
            ...
            function syncPekerjaanFromLansia() {
                const opt   = selectLansia?.selectedOptions?.[0];
                const raw   = opt?.dataset.pekerjaan || '';
                const label = decodePekerjaan(raw);
                const disp  = document.getElementById('ppok-pekerjaan-display');
                const hid   = document.getElementById('ppok-pekerjaan-hidden');
            ...
            selectLansia?.addEventListener('change', async function () {
                ...
                syncPekerjaanFromLansia();

                // PUMA kategori usia
                try {
                    const dob = opt?.dataset?.tanggalLahir || '';
                    const age = opt?.dataset?.umur || '';
                    const res = computePumaCategoryFromLansiaData(dob, age);
                    const dispEl = document.getElementById('puma-kategori-display');
                    const hidEl  = document.getElementById('puma-kategori-hidden');
                    if (dispEl) dispEl.value = res.label || '';
                    if (hidEl)  hidEl.value  = (res.cat !== null) ? res.cat : '';
                } catch (_) {}

                // Auto-fill tinggi badan
                if (id && tbTerakhirMap[id]) {
                    const tbEl = document.querySelector('[name="tinggi_badan"]');
                    if (tbEl && !tbEl.value) { tbEl.value = tbTerakhirMap[id]; hitungIMT(); }
                }
                ...
                saranSebelumnyaList.innerHTML =
                    ...
                    + '<i class="fa-solid fa-spinner fa-spin"></i> Memuat...</span>';
                ...
                try {
                    ...
                    if (data.length === 0) {
                        ...
                        saranSebelumnyaList.innerHTML = data.map(s => `
                            <div class="saran-edit-form" data-id="${s.id_saran}"
                                style="margin-bottom:16px;padding:12px 14px;background:var(--blue-50,#eff6ff);
                                       border:1.5px solid var(--blue-500,#3b82f6);border-radius:10px;">
                        ...
                        saranSebelumnyaList.querySelectorAll('.btn-delete-saran-lama')
                            .forEach(btn => btn.addEventListener('click', handleDeleteSaranLama));
                    }
                } catch (_) {
                    saranSebelumnyaWrapper.style.display = 'none';
                }
            ...
            function saranRowHTML(i) {
                return `
                    <div class="saran-row-inner" style="margin-bottom:10px;">
                        ...
                        <button type="button" class="btn-remove-resep btn-remove-saran"
                            title="Hapus saran" style="margin-left:8px;flex-shrink:0;margin-top:2px;">
            ...
            document.getElementById('btn-add-resep')?.addEventListener('click', () => {
                ...
                const row  = document.createElement('div');
                row.className = 'resep-row';
                row.style.cssText = 'margin-bottom:12px;border-bottom:1px dashed #ccc;padding-bottom:12px;';
                ...
                if (chkResep?.checked) {
                    row.querySelectorAll('select.form-control, input.form-control').forEach(el => {
                        const name = el.name || '';
                        if (name.includes('[id_obat]') || name.includes('[dosis]') ||
                            name.includes('[frekuensi]') || name.includes('[jumlah_obat]')) {
                            el.required = true;
                        }
                    });
                }
            ...
            function bindJenisJadwal(row) {
                ...
                const frekEl   = row.querySelector('.input-frekuensi');
                if (select && group) {
                    select.addEventListener('change', () => {
                        const isHariTertentu = select.value === 'hari_tertentu';
                        group.style.display = isHariTertentu ? 'flex' : 'none';
                        if (frekEl) frekEl.placeholder = isHariTertentu ? 'Frq/mgg' : 'Frq/hari';
                        if (!isHariTertentu && checkboxes) checkboxes.forEach(cb => cb.checked = false);
                    });
                    syncPekerjaanFromLansia();
                ...
                <div class="resep-row-inner" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;align-items:flex-end;">
                    <div style="grid-column:span 2;">
                        ...
                        <select name="resep[${i}][id_obat]" class="form-control" style="width:100%;">
                            <option value="">-- Pilih Obat --</option>${opts}
                        </select>
                    </div>
                    <div>
                    ...
                    <div style="display:flex;justify-content:flex-end;">
                        <button type="button" class="btn-remove-resep"
                            style="padding:8px 10px;background:#fee2e2;border:1px solid #fca5a5;color:#dc2626;border-radius:6px;cursor:pointer;width:100%;">
                            <i class="fa-solid fa-trash"></i> Hapus Baris
                        </button>
                    </div>
                </div>
                <div class="hari-konsumsi-group"
                    style="display:none;gap:10px;margin-top:10px;flex-wrap:wrap;background:#f8fafc;
            ...
            const paparanFrekGroup    = document.getElementById('paparan-frekuensi-group');
            ...
            function setRadioGroupDisabled(name, disabled, clear = false) {
                [...document.querySelectorAll(`input[name="${name}"]`)].forEach((radio, i) => {
                    radio.disabled = disabled;
                    radio.required = !disabled && i === 0;
                    if (disabled && clear) radio.checked = false;
                });
            }
            function setSelectState(el, disabled, clear = false) {
                if (!el) return;
                el.disabled = disabled;
                el.required = !disabled;
                if (disabled && clear) el.value = '';
            }
            function setTextState(el, disabled, clear = false) {
                if (!el) return;
                el.disabled = disabled;
                el.required = !disabled;
                if (disabled && clear) el.value = '0';
            }

            function syncPumaGenderFromLansia() {
                const gender = selectLansia?.selectedOptions?.[0]?.dataset?.jenisKelamin || '';
                if (!gender) {
                ...
                if (gender === 'L') {
                    ...
                    ivaGroup?.classList.add('disabled-locked');
                    setRadioGroupDisabled('iva_sadanis', true, true);
                } else {
            ...
            function hitungPackYears() {
                ...
                if (pyEl) pyEl.value = (rph > 0 && lmt > 0)
                    ? `${((lmt * rph) / 20).toFixed(2)} pack-years`
                    : '0 pack-years';
            }

            function syncPaparanFrekuensi() {
                const show = getCheckedValue('paparan_asap_rokok') === '1';
                if (paparanFrekGroup) {
                    paparanFrekGroup.style.display = show ? '' : 'none';
                    paparanFrekGroup.classList.toggle('disabled-locked', !show);
                }
                document.querySelectorAll('input[name="paparan_asap_rokok_frekuensi"]')
                    .forEach((r, i) => {
                        r.disabled = !show;
                        r.required = show && i === 0;
            ...
            function syncSmokingState() {
                const hasBoth    = Boolean(stepUtama && stepPpok);
                const utamaVal   = getCheckedValue('merokok');
                const ppokVal    = getCheckedValue('merokok_ppok');
                const sourceVal  = hasBoth ? utamaVal : (stepPpok ? ppokVal : utamaVal);
                const smoker     = sourceVal === '1';
                ...
                if (hasBoth) {
                    setRadioGroupValue('merokok_ppok', sourceVal);
    ...
    ['gangguan_pendengaran', 'disabilitas'].forEach(val => {
        ...
        if (el) el.disabled = false;
            ...
            [
                'merokok','paparan_asap_rokok','konsumsi_alkohol','konsumsi_gula','konsumsi_garam',
                'konsumsi_minyak','konsumsi_sayur_buah','aktivitas_fisik','kurang_aktivitas_fisik',
                'kurang_sayur_buah','merokok_ppok','konsumsi_alkohol_ppok','rapid_antigen',
                'puma_napas_pendek','puma_sulit_dahak','puma_batuk_tanpa_flu','puma_pernah_spirometri',
            ].forEach(name => setRadioGroupDisabled(name, false, false));

            selectLansia?.addEventListener('change', syncPumaGenderFromLansia);
            document.querySelectorAll('input[name="merokok"]')
                .forEach(r => r.addEventListener('change', syncSmokingState));
            document.querySelectorAll('input[name="merokok_ppok"]')
                .forEach(r => r.addEventListener('change', syncSmokingState));
            document.querySelectorAll('input[name="paparan_asap_rokok"]')
                .forEach(r => r.addEventListener('change', syncPaparanFrekuensi));
            ...
            //  SPIROMETRI RASIO
            // ══════════════════════════════════════════════════════════════
            function bindRasio(vep1Id, kvpId, previewId) {
                const vep1El    = document.getElementById(vep1Id);
                const kvpEl     = document.getElementById(kvpId);
                const previewEl = document.getElementById(previewId);
                function hitung() {
                    const v = parseFloat(vep1El?.value);
                    const k = parseFloat(kvpEl?.value);
                    if (previewEl) previewEl.value = (v > 0 && k > 0)
                        ? ((v / k) * 100).toFixed(2) + ' %' : '—';
                }
                vep1El?.addEventListener('input', hitung);
                kvpEl?.addEventListener('input', hitung);
            }
            bindRasio('vep1-pre',  'kvp-pre',  'preview-rasio-pre');
            bindRasio('vep1-post', 'kvp-post', 'preview-rasio-post');
            ...
            document.getElementById('formSkrining')?.addEventListener('submit', function (e) {
                if (!validateAllPanels()) { e.preventDefault(); return; }
                ...
                // Kumpulkan saran edit sebagai hidden input
                document.querySelectorAll('.saran-edit-form').forEach(form => {
                    ...
                    mk(`saran_edit[${id}][jenis_saran]`, jenis);
                    mk(`saran_edit[${id}][isi_saran]`,   isi);
                });
            });
