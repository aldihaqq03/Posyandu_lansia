...
document.addEventListener("DOMContentLoaded", function () {
            ...
            fetch(`/jadwal_posyandu/${id}`, {
                ...
                .then((data) => {
                    ...
                    document.getElementById("edit-chk-utama").checked =
                        data.ada_skrining_utama == 1;
                    document.getElementById("edit-chk-ppok").checked =
                        data.ada_skrining_ppok == 1;
    ...
    document
        ...
        ?.addEventListener("click", function () {
            ...
            const payload = {
                ...
                ada_skrining_utama: document.getElementById("edit-chk-utama")
                    .checked
                    ? 1
                    : 0,
                ada_skrining_ppok: document.getElementById("edit-chk-ppok")
                    .checked
                    ? 1
                    : 0,
                kegiatan: kegiatan,
    ...
    document
        ...
        ?.addEventListener("click", function () {
            ...
            const payload = {
                ...
                tema: tema,
                lokasi: lokasi,
                ada_skrining_utama: document.getElementById("chk-utama").checked
                    ? 1
                    : 0,
                ada_skrining_ppok: document.getElementById("chk-ppok").checked
                    ? 1
                    : 0,
                kegiatan: kegiatan,
                keterangan:
    ...
    // Keterangan: Menyaring tampilan jadwal langsung di browser
    // tanpa request ke server. Filter bekerja dengan cara
    // membaca data-* attribute pada setiap card jadwal,
    // lalu membandingkannya dengan input user (search, status, bulan).
    // Card yang tidak sesuai akan di-hide (display: none).
    // ========================================================
    ...
    function applyFilter() {
        ...
        // Loop semua card jadwal
        const allCards = document.querySelectorAll(".jadwal-card");
        allCards.forEach((card) => {
            const tema = (card.dataset.tema || "").toLowerCase();
            const lokasi = (card.dataset.lokasi || "").toLowerCase();
            const s = card.dataset.status || "";
            const b = card.dataset.bulan || "";

            // Cek apakah card cocok dengan filter
            const matchSearch =
                !search || tema.includes(search) || lokasi.includes(search);
            const matchStatus = !status || s === status;
            const matchBulan = !bulan || b === bulan;

            // Tampilkan hanya yang memenuhi semua filter
            if (matchSearch && matchStatus && matchBulan) {
                card.style.display = ""; // kembalikan ke display awal (flex/block dari CSS)
        ...
        // Handle month divider: sembunyikan jika semua card di bawahnya di-hide
        const allDividers = document.querySelectorAll(".month-divider");
        allDividers.forEach((divider) => {
            const bulanDivider = divider.dataset.bulan || "";

            // Cari semua card yang punya data-bulan sama dengan divider
            let hasVisibleCard = false;
            allCards.forEach((card) => {
                if (
                    card.dataset.bulan === bulanDivider &&
            ...
            // Jika ada card yang tampil, divider juga tampil
            if (hasVisibleCard) {
    ...
    // Pasang event listener: filter otomatis jalan saat input berubah
