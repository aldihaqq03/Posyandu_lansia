...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const namaTerpilihPlaceholder =
        document.querySelector(".form-title strong");
    ...
    if (tableBody) {
        tableBody.addEventListener("click", function (e) {
            ...
            if (
                clickedBtn &&
                (clickedBtn.classList.contains("btn-pilih") ||
                    clickedBtn.classList.contains("btn-terpilih"))
            ) {
                ...
                if (currentRow) {
                    ...
                    const namaLansia = currentRow.querySelector(
                        "td:nth-child(2) strong",
                    );
                    if (namaTerpilihPlaceholder && namaLansia) {
                        namaTerpilihPlaceholder.textContent =
                            namaLansia.textContent;
                    }
                }
            }
        });
    ...
    if (formPemeriksaan) {
        formPemeriksaan.addEventListener("submit", function (e) {
            ...
            const namaLansia = namaTerpilihPlaceholder
                ? namaTerpilihPlaceholder.textContent
                : "Lansia";
            alert(
                `Data pemeriksaan kesehatan untuk ${namaLansia} berhasil disimpan! (Simulasi)`,
            );
