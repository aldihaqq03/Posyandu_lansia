...
class SkriningController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (in_array(DetailSkrining::SKRINING_PPOK, $aktifSkrining)) {
            ...
            if ($request->filled('puma_jenis_kelamin') && (int) $request->puma_jenis_kelamin !== $pumaJenisKelamin) {
                throw ValidationException::withMessages([
                    'puma_jenis_kelamin' => ['Jenis kelamin PUMA tidak sesuai dengan data lansia.'],
                ]);
            }

            if (in_array(DetailSkrining::SKRINING_UTAMA, $aktifSkrining) && $request->filled('merokok') && $request->filled('merokok_ppok') && $request->merokok !== $request->merokok_ppok) {
                throw ValidationException::withMessages([
        ...
        DB::transaction(function () use ($request, $jadwal, $aktifSkrining, $lansiaTerpilih) {
            ...
            if (in_array(DetailSkrining::SKRINING_PPOK, $aktifSkrining)) {
                ...
                $riwayatKeluargaPPOK = $request->riwayat_penyakit_keluarga_ppok
                    ? json_encode($request->riwayat_penyakit_keluarga_ppok) : null;
                $riwayatSendiriPPOK = $request->riwayat_penyakit_sendiri_ppok
            ...
            try {
                $userLansia = User::whereHas('lansia', function ($q) use ($request) {
            ...
            Cache::forget('dash_total_lansia');
            Cache::forget('dash_resiko_tinggi');
            Cache::forget('dash_pemeriksaan_selesai');
            Cache::forget('dash_penyakit_counts');
            Cache::forget('dash_riwayat_terakhir');
            Cache::forget('dash_lansia_checked');
